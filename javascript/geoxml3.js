...
var coordListA = [];
  ...
  var render = function (responseXML, doc) {
    ...
    doc.styles = styles;
      ...
      var placemarkNodes = responseXML.getElementsByTagName('Placemark');
        //搭配oms.js實現Marker Spiderfier 解決標紀重疊問題
      var gm = google.maps;
      ...
      oms.addListener('click', function (marker) {
          var a = marker.desc;
          var b = a.split('@')
          iw.setContent('<div class="geoxml3_infowindow"><h3>' + b[0] +
       '</h3><div>' + b[1] + '</div></div>');
      ...
      if (!!window.google && !!google.maps) {
       if (placemark.Point) {
          ...
          if (!!parserOptions.createMarker) {
            ...
            parserOptions.createMarker(placemark, doc,oms,gm,iw);
            ...
            if (!found) {
              ...
              marker = createMarker(placemark, doc,oms,gm,iw);
              marker.active = true;
            }
          }
         }
  ...
  var createMarker = function (placemark, doc,oms,gm,iw) {
    ...
    var marker = new gm.Marker(markerOptions);
    if (!!doc) {
    ...
    if (!parserOptions.suppressInfoWindows) {
        marker.desc = placemark.name +"@"+ placemark.description;
